...
for_plotting = for_plotting.groupby(
    ['Year', 'gender', 'type_cat']).sum().drop('Rate', axis=1).reset_index()
...
for_plotting['proportion_Funded'] = for_plotting['Funded'] / \
    for_plotting['Applications'] * 100
total_funded = for_plotting.groupby(['Year', 'type_cat']).sum()[
    'Funded'].to_dict()
total_amounts = for_plotting.groupby(['Year', 'type_cat']).sum()[
    'Amount'].to_dict()

for_plotting['mapper'] = tuple(
    zip(for_plotting['Year'], for_plotting['type_cat']))
...
for_plotting['proportion_amount'] = for_plotting['Amount'] / \
    for_plotting['total_amount'] * 100
...
sns.barplot(x='Year_num',  y='Amount', data=df,
            hue='gender', ax=ax, palette=col_pal)
legend_elements = [Patch(facecolor=col_pal[x], label=labels[x])
                   for x in range(0, len(labels))]
ax.legend(handles=legend_elements, loc='upper left',
          title='Funding Amount', ncol=3)
ax2 = ax.twinx()
...
plt.legend(labels, loc='upper left', title='Number funded',
           ncol=3, bbox_to_anchor=(0.67, 1.0))
...
for level, df in for_plotting.groupby('type_cat'):
    ...
    m = sns.barplot(orient='h', y=list(plotting['Year_num']), x=[
                    100 for x in plotting['Year_num']], color=male_colour)
    f = sns.barplot(x=plotting['proportion_total_funded'],
                    y=plotting['Year_num'], color=fem_colour, orient='h')
